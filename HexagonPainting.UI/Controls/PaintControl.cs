...
namespace HexagonPainting.UI.Controls;

public class PaintControl : Control, IPointer
{
    ...
    private void PaintControl_PointerMoved(object? sender, PointerEventArgs e)
    {
        if (Vm != null)
        {
            ...
            Ioc.Default.GetService<IPointer>();
            var pos = e.GetPosition(this);
            _mousePosition = new Vector2((float)pos.X, (float)pos.Y);
            if (Vm.Dragging)
            {
                var mainLayer = Ioc.Default.GetService<Layer<Color>>();
                mainLayer.Draw();
            }
