...
namespace HexagonPainting.ViewModels
{
    public class PaintControlViewModel : INotifyPropertyChanged
    {
        ...
        {
            RenderTargetBitmap newImage =
                new RenderTargetBitmap(new PixelSize(size.Width, size.Height), new Vector(96, 96));
            if (_image != null)
            {
                // If there was already a bitmap, copy it into the new one.
                using (var context = newImage.CreateDrawingContext())
                {
                    context.DrawImage(_image, new Rect(0, 0, size.Width, size.Height),
        ...
        private void DrawHexagon(DrawingContext context, Hex hex)
        {
            ...
            var pen = new Pen(new SolidColorBrush(hex.Color));
            var brush = new ImmutableSolidColorBrush(new SolidColorBrush(hex.Color));
